...
class Ndef {
  ...
  Future<NdefMessage> read() async {
    return channel.invokeMethod('Ndef#read', {'handle': _tag.handle}).then((value) => $GetNdefMessage(Map.from(value)));
  }

  /// Write the NDEF message on this tag.
  Future<void> write(NdefMessage message) async {
    return channel.invokeMethod('Ndef#write', {'handle': _tag.handle, 'message': $GetNdefMessageMap(message)});
  }
  ...
  Future<void> writeLock() async {
    return channel.invokeMethod('Ndef#writeLock', {'handle': _tag.handle});
  }
}
...
class NdefMessage {
  ...
  int get byteLength => records.isEmpty ? 0 : records.map((e) => e.byteLength).reduce((a, b) => a + b);
}
...
class NdefRecord {
  ...
  // NdefRecord
  const NdefRecord._({required this.typeNameFormat, required this.type, required this.identifier, required this.payload});
  ...
  factory NdefRecord.createUri(Uri uri) {
    ...
    int prefixIndex = uriPrefixList.indexWhere((e) => uriString.startsWith(e), 1);
    ...
    return NdefRecord(
      ...
      identifier: Uint8List.fromList([]),
      payload: Uint8List.fromList([prefixIndex] + utf8.encode(uriString.substring(uriPrefixList[prefixIndex].length))),
    );
  }
  ...
  // _validateFormat
  static void _validateFormat(NdefTypeNameFormat format, Uint8List type, Uint8List identifier, Uint8List payload) {
    switch (format) {
